...
// home page route showing the last 5 events
router.get("/event", function (req, res) {
    db.Events.find({}).then((response) => {
        // response is an array, so I'm slicing off the last 5 items
...
router.put("/event/:id", parser.single("image"), function (req, res) {
    db.Events.findById(req.params.id)
        .then(event => {
            ...
            // if a new image is being uploaded to an event
            if (req.file) {
            ...
            db.Events.findByIdAndUpdate(event._id,
                {
                    // we assume that all the fields can be updated except organizer
                    $set: {
                        name: req.body.name,
